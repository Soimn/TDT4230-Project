...
main :: proc()
{
	...
	handle, _ := os.open(cwd);

	file_infos, _ := os.read_dir(handle, 1000);
	
	for file_info in file_infos
	{
		obj_filename := file_info.name;
		if !strings.has_suffix(obj_filename, ".obj") do continue;

		obj_file: string;

		if obj_file_data, ok := os.read_entire_file_from_filename(obj_filename); ok do obj_file = string(obj_file_data);
		else
		{
			fmt.println("ERROR: Failed to read", obj_filename);
			return;
		}
		...
		filename := strings.concatenate([]string{strings.trim_suffix(obj_filename, ".obj"), ".scene"});
